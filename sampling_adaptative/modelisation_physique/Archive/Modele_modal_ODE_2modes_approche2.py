...
Y_m2 = 1 /200                  #Admittance au deuxième mode
...
def funtion(X,args):
    ...
    X2=[X[1], F1*(X[1]+X[3])*(A+2*B*(X[0]+X[2])+3*C*(X[0]+X[2])**2)-Y_m1*X[1] - omega1**2*X[0],X[3],F2*(X[1]+X[3])*(A+2*B*(X[0]+X[2])+3*C*(X[0]+X[2])**2)-Y_m2*X[3] - omega2**2*X[2]]
...
X=np.array([gamma,0,gamma,0]) #Pour deux modes
p=RK4(X,args)
tcalc=time.time()-t1
