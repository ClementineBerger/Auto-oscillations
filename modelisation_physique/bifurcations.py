...
for g in gammas_bif:                                
    p,time = Modele_modal_fct_rampe.simulation(tmax = tmax,
                                               nb_mode = 5,                       # Lancement du modèle désiré
                                               instrument = 'clarinette', 
                                               sample_rate = sample_rate, 
                                               gamma_velo = g, 
                                               zeta_force = zeta_force,
                                               durete_rampe = 2000, 
                                               l_resonateur = 60e-2,
    ...
    maxima = argrelextrema(p[int(len(p) - sample_rate*0.1*tmax):],        # Obtention des maxima
                           np.greater, 
                           order = 15
                           )                                                    
